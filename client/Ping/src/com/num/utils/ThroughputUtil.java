...
public class ThroughputUtil {
	...
	public ThroughputUtil(Context context){
		...
		broadcastReceiver = new ThroughputBroadcastReceiver();
	...
	private class ThroughputBroadcastReceiver extends BroadcastReceiver {
			...
			if (parcels != null) {
				...
				for (int i = 0; i < results.length; i++) {
					...
					if (desc.dir_up) {
						...
						down_done = true;
						DonwlinkToast(downlink.getDstIp());
					}
				}
			}
		}
		
		private void UplinkToast(String dstIp) {
			Toast.makeText(mContext,"Uplink test to " + dstIp + " complete!", Toast.LENGTH_LONG).show();
		}
		
		private void DonwlinkToast(String dstIp) {
			Toast.makeText(mContext, "Downlink test to " + dstIp + " complete!", Toast.LENGTH_LONG).show();
		}
	}
	
	
	// Just moving to here from Uplink method, it seems to be sth related with SouthAfrica measurements
	...
	//So does here for downlink measurement ---------------------------
	/*Values session = (Values) context.getApplicationContext();
	DeviceUtil device = new DeviceUtil();
	String countrycode = device.getNetworkCountry(context);
	String serveraddress=session.THROUGHPUT_SERVER_ADDRESS;
