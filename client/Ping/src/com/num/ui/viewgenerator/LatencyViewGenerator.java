	...
	public void populateView(Row item,final Context context,View view) {
		...
		holder.linear.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				picker.filterBy(LatencyMapping.COLUMN_TYPE,ping.getPing().getDst().getType(),"Type");
				picker.filterBy(LatencyMapping.COLUMN_DSTIP,ping.getPing().getDst().getTagname(),"Destination");
				picker.filterBy(LatencyMapping.COLUMN_MEASUREMENT,"average","Metric");
				picker.filterBy(LatencyMapping.COLUMN_CONNECTION,DeviceUtil.getNetworkInfo(context),"Connection");
